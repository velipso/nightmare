...
static sink_val L_help(sink_ctx ctx, int size, sink_val *args, void *user){
	return sink_str_newcstr(ctx,
		...
		"  bake T             # send the events within `T` ticks to the engine\n"
		"  bakeall            # send all pending events to the engine\n"
		"  reset T[, tpq]     # insert reset event with ticks per quarternote\n"
		"  defpatch p, wave, peak, attack, decay, sustain, harmonic1, h2, h3, h4\n"
		"                     # overwrite the default patch parameters for patch p\n"
		"  mastervolume T, v  # insert master volume event\n"
		"  masterpan T, p     # insert master pan event, -1 left, 0 center, 1 right\n"
		"  channel [c]        # select a channel for future commands\n"
		"  volume T, v        # insert volume event for the current channel\n"
		"  expression T, e    # fraction of volume to use\n"
		"  pan T, p           # insert pan event, -1 left, 0 center, 1 right\n"
		...
		"  bend T, b          # pitch bend the current channel by b semitones\n"
		"  patch T, p         # switch patch for the current channel to p (number)\n"
		...
		"  tempo T, n, d, bpm # insert tempo event with n/d time signature and BPM speed\n"
		"\nExample:\n"
		"> reset      0, 50         # 50 ticks per quarternote\n"
		"> tempo      0, 4, 4, 120  # 4/4 timesig with 120 beats per minute\n"
		"> play   0, 100, C4        # play C4 starting at tick 0 for 100 ticks\n"
		"> play 100, 100, D4        # play D4 starting at tick 100 for 100 ticks\n"
		"> play 200, 100, E4        # play E4 starting at tick 200 for 100 ticks\n"
		"> bakeall                  # send all pending events to the engine"
	);
}
